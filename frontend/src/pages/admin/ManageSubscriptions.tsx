...
export default function ManageSubscriptions() {
  ...
  const savePrice = (planId: string) => {
    ...
    if (Number.isNaN(value) || value < 0) {
      toast(t('admin.subs.invalidPrice'), 'error')
      return
    }
    const plan = db.getPlan(planId)!
    if (value === plan.price) return
    db.updatePlan(planId, { price: value }, actor)
    toast(t('admin.subs.priceUpdated', { plan: plan.name, price: value.toFixed(2) }), 'success')
    refresh()
  }
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-xl font-bold text-ink">{t('admin.subs.title')}</h1>

      {/* Summary strip */}
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: t('admin.subs.active'), value: active, tone: 'text-status-success' },
          { label: t('admin.subs.expiring'), value: expiringSoon, tone: 'text-status-warning' },
          { label: t('admin.subs.expired'), value: expired, tone: 'text-status-danger' },
        ].map((s) => (
      ...
      <div className="grid gap-3 md:grid-cols-4">
        {plans.map((p) => (
          <div key={p.id} className={`card relative p-4 ${p.popular ? 'border-primary' : ''}`}>
            {p.popular && (
              <span className="absolute -top-2.5 right-3 rounded-[3px] bg-accent px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.1em] text-ink">{t('admin.subs.popular')}</span>
            )}
            ...
            <div className="mt-2 space-y-2">
              <div>
                <label className="label" htmlFor={`price-${p.id}`}>{t('admin.subs.priceUsd')}</label>
                <input
                  id={`price-${p.id}`}
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-ink-soft">
                <p>{t('admin.subs.duration')} <span className="font-semibold text-ink">{t('admin.subs.days', { count: p.durationDays })}</span></p>
                <p>{t('admin.subs.limit')} <span className="font-semibold text-ink">{p.bookLimit == null ? t('admin.subs.unlimited') : p.bookLimit}</span></p>
              </div>
              <button
                className="text-xs font-semibold text-primary"
                onClick={() => savePrice(p.id)}
              >
                {t('admin.subs.savePrice')}
              </button>
            </div>
          </div>
        ))}
        <div className="card flex min-h-40 flex-col items-center justify-center gap-1 border-dashed p-4 text-center text-ink-faint">
          ...
          <span className="text-xs font-semibold">{t('admin.subs.newPlans')}</span>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="border-b border-divider px-4 py-3">
          <h2 className="section-title">{t('admin.subs.allSubs', { count: subs.length })}</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-180 text-sm">
            <thead>
              <tr className="border-b border-divider bg-canvas text-left text-[11px] uppercase tracking-wide text-ink-faint">
                <th className="px-4 py-2.5 font-semibold">{t('admin.subs.user')}</th>
                <th className="px-4 py-2.5 font-semibold">{t('admin.subs.plan')}</th>
                <th className="px-4 py-2.5 font-semibold">{t('admin.subs.started')}</th>
                <th className="px-4 py-2.5 font-semibold">{t('admin.subs.expires')}</th>
                <th className="px-4 py-2.5 font-semibold">{t('admin.subs.ref')}</th>
                <th className="px-4 py-2.5 font-semibold">{t('admin.subs.status')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-divider">
              ...
              {subs.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-ink-faint">{t('admin.subs.empty')}</td></tr>
              )}
            </tbody>
          </table>
        </div>
