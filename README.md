...
| `backend/` | 🚧 Scaffold only | Express + TS config (`config/env`, `config/database`) and middleware (`auth`, `errorHandler`) in place. No `server.ts`, no API routes, no DB schema yet |
...
| `storage/` | ⏳ Planned | Private PDF storage (never public) — not started |
...
# Backend scaffold (no runnable server yet — needs MySQL, see backend/.env.example)
cd backend
npm install
cp .env.example .env # fill in DB + JWT secrets
npm run dev          # ⚠️ no src/server.ts yet — API routes/DB schema are next
```
...
- **Backend (scaffold):** Node.js, Express 4, TypeScript · JWT + bcryptjs · zod validation · helmet + rate limiting · mysql2 pool (`backend/package.json`)
...
- [ ] Backend API: `server.ts`, auth, books, subscriptions, payments, reader tokens
